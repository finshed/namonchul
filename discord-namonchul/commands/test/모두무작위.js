...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("모두무작위")
    .setDescription("라인과 챔프를 모두 정해드립니다 (무작위)"),
  /**
   *
  ...
  async execute(interaction) {
    //라인 정하기
     const lines = ["탑","미드","정글","원딜","서폿"] //괄호안에 메세지 더 추가하시거나 수정하시면됩니다
      const randomlines = Math.floor(Math.random() * lines.length)
    //챔프 정하기 (12.20 버전)
      ...
      const randomChampion = Math.floor(Math.random() * Champ.length)
      
      await interaction.reply({ content: `라인은 ${lines[randomlines]} 챔프는 ${Champ[randomChampion]} :heart_eyes: ` })
